export default async function handler(request, response) {
  ...
  const { topicZh = '', topicFr = '', questions = [] } = request.body || {};
  const prompt = `你是TCF Canada口语Tâche 2法语考官。请用中文反馈，并保留法语修改。根据原题逐一审核问题是否切题、语法正确、表达自然；为不自然或错误的问题给出更地道的法语改写；最后给出100分总分、覆盖度和缺失追问角度。\n中文题意：${topicZh}\n法语原题：${topicFr}\n问题：\n${questions.map((q,i)=>`${i+1}. ${q}`).join('\n')}`;
