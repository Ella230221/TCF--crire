export default async function handler(request, response) {
  ...
  const { topicFr = '', questions = [], conversation = [] } = request.body || {};
  const prompt = `你是TCF Canada口语Tâche 2法语考官。请用中文反馈，并保留法语修改。审核考生准备的完整对话，而不只是问句：检查是否切题、语法是否正确、开场和礼貌表达、解释和回应、过渡与追问是否自然，是否像真人交流而非连续审问。逐句给出更地道的法语改写，最后给出100分总分、问题覆盖度、互动自然度和缺失角度。\n法语原题：${topicFr}\n完整对话：\n${(conversation.length?conversation:questions).map((line,i)=>`${i+1}. ${line}`).join('\n')}`;
