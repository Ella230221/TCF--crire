(() => {
  ...
  const moduleStorageKey = 'tcf-t3-editable-modules-v1';
  ...
  try {
    ...
    Object.entries(edits).forEach(([id,html])=>{const section=document.getElementById(id);if(section) section.innerHTML=html;});
  } catch (_) {}
  document.querySelectorAll('main .section p, main .section li, main .section h3').forEach(node=>{ if(!node.closest('.t3-import-panel')) node.contentEditable='true'; });
  let moduleTimer;
  document.querySelector('main').addEventListener('input',event=>{
    ...
    moduleTimer=setTimeout(()=>{
      const edits={};
      document.querySelectorAll('main > .section[id]:not(#import-t3)').forEach(section=>{edits[section.id]=section.innerHTML;});
